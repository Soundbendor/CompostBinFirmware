"""
Unit tests for the split helper modules.

These tests mirror the monolithic helpers characterization coverage while
patching the focused module namespaces introduced beside helpers/__init__.py.
"""
...
# Stub module types used before importing the helpers package.
...
import helpers.calibration_loader as calibration_loader
import helpers.logging_helper as logging_helper
import helpers.request_handler as request_handler
import helpers.time_helper as time_helper
from helpers.calibration_loader import CalibrationLoader
from helpers.logging_helper import Logging
...
# Split TimeHelper should seed lastTime from the current clock value
def test_split_time_helper_initializes_last_time_from_time():
    with patch("helpers.time_helper.time", return_value=123.4):
        helper = TimeHelper()

    assert helper.lastTime == 123.4


# Non-multiple timestamps should not trigger the split time interval helper
def test_split_two_hour_interval_returns_false_when_not_at_multiple():
    helper = TimeHelper.__new__(TimeHelper)
    helper.lastTime = 1

    with patch(
        "helpers.time_helper.time",
        return_value=time_helper.TWO_HOURS_SECONDS + 1,
    ):
...
# Multiple timestamps should trigger once and suppress duplicate interval reads
def test_split_two_hour_interval_returns_true_once_per_multiple_timestamp():
    helper = TimeHelper.__new__(TimeHelper)
    helper.lastTime = 0

    with patch("helpers.time_helper.time", return_value=time_helper.TWO_HOURS_SECONDS):
        assert helper.twoHourInterval() is True
        assert helper.twoHourInterval() is False

    assert helper.lastTime == time_helper.TWO_HOURS_SECONDS


# Split Logging should default to console INFO logging without an output file
def test_split_logging_configures_info_console_logging_by_default():
    with patch("helpers.logging_helper.sys.argv", ["main.py"]):
        with patch("helpers.logging_helper.logging.basicConfig") as basic_config:
            with patch("helpers.logging_helper.logging.info") as log_info:
                Logging()
...
# Split Logging should use WARNING console logging when verbose mode is off
def test_split_logging_configures_warning_console_logging_when_not_verbose():
    with patch("helpers.logging_helper.sys.argv", ["main.py"]):
        with patch("helpers.logging_helper.logging.basicConfig") as basic_config:
            with patch("helpers.logging_helper.logging.info"):
...
# Split Logging should attach file and stream handlers when a log path exists
def test_split_logging_configures_file_and_stream_handlers_when_log_path_supplied():
    fake_file_handler = Mock(name="file_handler")
    fake_stream_handler = Mock(name="stream_handler")

    with patch("helpers.logging_helper.sys.argv", ["main.py", "bucket.log"]):
        with patch(
            "helpers.logging_helper.logging.FileHandler",
            return_value=fake_file_handler,
        ) as file_handler:
            with patch(
                "helpers.logging_helper.logging.StreamHandler",
...
# Split CalibrationLoader should load JSON data and return requested fields
def test_split_calibration_loader_loads_json_and_returns_fields():
    calibration_data = {"NAU7802_CALIBRATION_FACTOR": 42.5}

    with patch(
        "helpers.calibration_loader.open",
        mock_open(),
        create=True,
    ) as open_mock:
        with patch(
            "helpers.calibration_loader.json.load",
...
# Split CalibrationLoader should keep KeyError behavior for missing fields
def test_split_calibration_loader_missing_field_raises_key_error():
...
# Split RequestHandler construction should load credentials, email, and serial state
def test_split_request_handler_constructor_loads_credentials_and_formats_endpoint():
    with patch.object(
        RequestHandler,
        "loadFastAPICredentials",
        return_value=("api-key", "api.example.test", 8443),
    ) as load_api:
        with patch.object(
            RequestHandler,
            "loadEmailCredentials",
            return_value=("app-password", "alerts@example.test"),
        ) as load_email:
            with patch.object(
                RequestHandler,
                "_getSerial",
                return_value="serial-123",
            ) as get_serial:
                with patch("helpers.request_handler.logging.basicConfig"):
...
# Split RequestHandler should mark email credentials missing for FAILED values
def test_split_request_handler_constructor_marks_email_credentials_missing():
    with patch.object(
        RequestHandler,
        "loadFastAPICredentials",
        return_value=("api-key", "api.example.test", 8443),
...
# Split FastAPI credential loading should read the configured secret file fields
def test_split_load_fast_api_credentials_reads_secret_file():
    handler = RequestHandler.__new__(RequestHandler)
    secret_data = {
        "FASTAPI_CREDS": {
            "apiKey": "api-key",
    ...
    with patch(
        "helpers.request_handler.open",
        mock_open(),
        create=True,
    ) as open_mock:
        with patch(
            "helpers.request_handler.json.load",
...
# Split updateAPICreds should reload secret values and rebuild the endpoint
def test_split_update_api_creds_reloads_credentials_and_formats_endpoint():
    ...
    with patch.object(
        handler,
        "loadFastAPICredentials",
        return_value=("new-key", "new-api.example.test", 9443),
    ) as load_api:
...
# Split network checking should return true when the socket connects
def test_split_check_network_connection_returns_true_on_socket_connect():
    handler = build_request_handler()
    fake_socket = Mock()

    with patch("helpers.request_handler.socket.setdefaulttimeout") as set_timeout:
        with patch(
            "helpers.request_handler.socket.socket",
    ...
    set_timeout.assert_called_once_with(5)
    socket_ctor.assert_called_once_with(
        request_handler.socket.AF_INET,
        request_handler.socket.SOCK_STREAM,
    )
    fake_socket.connect.assert_called_once_with(("1.1.1.1", 53))


# Split network checking should log and return false on socket errors
def test_split_check_network_connection_returns_false_on_socket_error(caplog):
    handler = build_request_handler()
    fake_socket = Mock()
    fake_socket.connect.side_effect = request_handler.socket.error("boom")

    with patch("helpers.request_handler.socket.socket", return_value=fake_socket):
...
# Split getAPIKey should expose the currently loaded API token.
def test_split_get_api_key_returns_current_key():
    handler = build_request_handler()

    assert handler.getAPIKey() == "api-token"


# Split heartbeat success should retry email credentials after network recovery
def test_split_send_heartbeat_returns_true_and_retries_email_credentials():
    handler = build_request_handler()
    ...
    with patch(
        "helpers.request_handler.httpx.Client",
        return_value=fake_client,
    ) as client_ctor:
        with patch.object(
            handler,
            "loadEmailCredentials",
            return_value=("new-password", "new-email@example.test"),
        ) as load_email:
...
# Split heartbeat should return false for missing or negative health responses
@pytest.mark.parametrize("response_json", [{}, {"is_alive": False}])
def test_split_send_heartbeat_returns_false_when_response_is_not_alive(response_json):
    handler = build_request_handler()
    fake_client = Mock()
    fake_client.get.return_value.json.return_value = response_json

    with patch("helpers.request_handler.httpx.Client", return_value=fake_client):
        result = handler.sendHeartbeat()
...
# Split heartbeat should log and return false when the HTTP client raises
def test_split_send_heartbeat_returns_false_on_http_exception(caplog):
    handler = build_request_handler()
    fake_client = Mock()
...
# Split secure heartbeat should send the API token to the secure endpoint
def test_split_send_secure_heartbeat_sends_token_header_and_returns_true():
    handler = build_request_handler()
    fake_client = Mock()
    fake_client.get.return_value.json.return_value = {"is_alive": True}
...
# Split secure heartbeat should fail for missing or negative health responses
@pytest.mark.parametrize("response_json", [{}, {"is_alive": False}])
def test_split_send_secure_heartbeat_returns_false_when_response_is_not_alive(
    response_json,
):
    handler = build_request_handler()
    fake_client = Mock()
...
# Split upload should build the payload, attach files, and return success
def test_split_send_api_request_posts_expected_payload_and_returns_success():
    handler = build_request_handler()
...
# Split upload should return API-level failure responses without raising
def test_split_send_api_request_returns_false_when_api_status_is_false():
    handler = build_request_handler()
    ...
    with patch(
        "helpers.request_handler.open",
        side_effect=[Mock() for _ in range(5)],
        create=True,
    ):
        with patch("helpers.request_handler.httpx.Client", return_value=fake_context):
...
# Split upload should return the standard error tuple when posting raises
def test_split_send_api_request_returns_error_tuple_when_http_post_raises(caplog):
    handler = build_request_handler()
    fake_client = Mock()
...
# Split support email should authenticate and send the formatted message
def test_split_send_error_email_logs_in_sends_message_and_returns_true():
    handler = build_request_handler()
    fake_smtp = Mock()
    fake_context = MagicMock()
    ...
    with patch(
        "helpers.request_handler.smtplib.SMTP_SSL",
        return_value=fake_context,
    ) as smtp_ssl:
...
# Split support email should log and return false when SMTP setup fails
def test_split_send_error_email_returns_false_on_smtp_failure(caplog):
    handler = build_request_handler()
...
# Split AWS secret loading should return notification email and app password
def test_split_load_email_credentials_returns_aws_secret_values():
    handler = RequestHandler.__new__(RequestHandler)
    fake_client = Mock()
    fake_session = Mock()
    ...
    with patch.object(
        request_handler.botocore,
        "session",
        SimpleNamespace(get_session=Mock(return_value=fake_session)),
        create=True,
    ) as botocore_session:
        with patch(
            "helpers.request_handler.SecretCacheConfig",
            return_value=fake_cache_config,
        ) as cache_config:
            with patch(
                "helpers.request_handler.SecretCache",
...
# Split AWS failures should return FAILED email credential sentinels
def test_split_load_email_credentials_returns_failed_values_on_exception(caplog):
    handler = RequestHandler.__new__(RequestHandler)
...
# Split serial lookup should parse the Raspberry Pi serial from cpuinfo
def test_split_get_serial_parses_cpuinfo_serial_line():
    handler = RequestHandler.__new__(RequestHandler)
...
# Split serial lookup should return the current error sentinel on cpuinfo failure
def test_split_get_serial_returns_error_value_when_cpuinfo_fails():
    handler = RequestHandler.__new__(RequestHandler)

    with patch(
        "helpers.request_handler.open",
        side_effect=OSError("no cpuinfo"),
        create=True,
    ):
        result = handler._getSerial()
